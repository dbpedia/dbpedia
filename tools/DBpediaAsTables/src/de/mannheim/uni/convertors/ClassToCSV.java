...
public class ClassToCSV {
	...
	// public static final String GET_INSTANCES_OF_CLASS =
	// "select distinct ?Concept FROM <http://dbpedia.org> where {?Concept a ?type}";
	...
	private class ClassToCSVThread implements Runnable {
		 ...
		 */
		private void writeToCSV(CSVWriter writer,
				List<DBpediaProperty> properties,
		...
		private List<String> getInstancesFromClass(String classURI) {
			...
			while (true) {
				...
				while (RS.hasNext()) {
					...
					if (!instancesURIs.contains(instanceName))
			...
			if (instance == null) {
				...
				while (true) {
					while (RS.hasNext()) {
						...
						if (propUri.contains("http://dbpedia.org/property/"))
							continue;
	...
	public static void removeDoneCLasses(List<String> allClasses) {
		...
		for (File fileEntry : folder.listFiles()) {
			if (allClasses.contains("http://dbpedia.org/ontology/"
					+ fileEntry.getName().replace(".csv.gz", "")))
				allClasses.remove("http://dbpedia.org/ontology/"
						+ fileEntry.getName().replace(".csv.gz", ""));
			if (fileEntry.getName().replace(".csv.gz", "").equals("Thing")
					&& allClasses
