...
while ($row = mysql_fetch_array($dbresult, MYSQL_ASSOC)) {
    ...
    $query = "select * from page where page_namespace = 10 and page_is_redirect = 0 and lower(page_title) LIKE '$template'";
    ...
    while ($row_template = mysql_fetch_array($dbresult_template, MYSQL_ASSOC)) {
        ...
        if (mysql_num_rows($dbresult_template) > 1) {
            echo "For template $template there is more than one entry which is not a redirect: ".$row_template["page_title"], PHP_EOL;
        }
        mysql_select_db($db);
    ...
    foreach ($pageTitles as $id => $pageTitle) {
        ...
        if ($row_count > 0) {
            $output = "{{ DBpediaConditionalMapping\n| cases =\n";
            while ($row_rules = mysql_fetch_row($result)) {
                $output .= " {{ DBpediaCondition";
                ...
                foreach ($template_properties as $template_property_id => $template_property) {
                    ...
                    while ($row_class_property = mysql_fetch_array($dbresult2, MYSQL_ASSOC)) {
                        ...
                        if (!in_array($template_property_id, $template_properties_to_merge)) {
                            $output .= "\t{{ DBpediaPropertyMapping | templateProperty = $template_property ";
                            if ($class && $property_name) {
                                $output .= "| ontologyProperty = $class/$property_name ";
                            } else {
            ...
            foreach ($merging_group as $template_properties) {
                ...
                foreach ($template_properties as $id => $template_property_id) {
                    ...
                    if ($parser_rule == "unit") {
                        if ($unit_type == "Length") {
                            if ($id == 0) {
                                $output .= "\t{{ DBpediaCalculateMapping | operation = add";
                                $brackets_open = true;
                            }
                        ...
                        if ($id == 0) {
                            $output .= "\t{{ DBpediaGeocoordinatesMapping";
                            $brackets_open = true;
                        ...
                        if ($id == 0) {
                            $output .= "\t{{ DBpediaCombineDateMapping";
                            $brackets_open = true;
