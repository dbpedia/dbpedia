...
/**
 * I just took the sybase dialect and started making modification so the TemplateDb
 * would run on virtuoso.
 * 
 * So far the following changes were made (in comparision with the original dialect):
 * .) @@identity was replaced with identity_value()
 * .) supportsInsertSelectIdentity set to false (thus invalidating appendIdentitySelectToInsert)
 * .) boolean now maps to smallint (as tinyint not supported)
 * 
 */
...
public class VirtuosoSybaseDialect extends Dialect {
