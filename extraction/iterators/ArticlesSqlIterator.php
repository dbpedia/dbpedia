...
class ArticlesSqlIterator extends AbstractMySqlIterator
{
	...
	protected function template($language) {
		if($language == 'en') {
			...
			return "select replace(trim(ll_title), ' ', '_') as page_title from page p inner join langlinks ll on p.page_id = ll.ll_from where (p.page_namespace = 0) and p.page_is_redirect = 0 and ll.ll_lang = '$language' LIMIT %d, %d";
		}
	}
