 ...
 * Outputs wgs84_pos latitude and longitude information along with appropriate feature details of the geonames ontology
 * 
...
class GeoExtractor implements Extractor 
{
    const extractorID = "http://dbpedia.org/extractors/GeoExtractor";
     ...
     * Urlencode UTF-8 if neccessary
     */
     var $knownTemplatePredicates = array('coordinates', 'location', 'lat_long', 'coords', 'place', 'Coor dms', 'coordonn%C3%A9es');
        ...
        foreach ($foundCoordinates as $g)  {
             if ($g->is_valid()) {
                ...
                if (GeoExtractor::enablePreview) {
                     ?>
                     <iframe src="http://christianhbecker.com/da/workspace/gmap.php?lat=<?=$g->latdeg?>&long=<?=$g->londeg?>&scale=5" style="width: 500px; height: 300px; border: none;" scrolling="no">
                     </iframe>
                     <?php
                }
                ...
                if (++$numResults == 1)  {
                    $result->addTriple(
                            ...
                            RDFtriple::Literal((string) $g->latdeg, "http://www.w3.org/2001/XMLSchema#float",NULL));

                    $result->addTriple(
                            RDFtriple::page($pageID), 
                            RDFtriple::URI("http://www.w3.org/2003/01/geo/wgs84_pos#long"),
                            RDFtriple::Literal((string) $g->londeg, "http://www.w3.org/2001/XMLSchema#float",NULL));
                    ...
                    if (isset($g->attr['type']))  {
                        if (isset($this->typeToGeoNames[$g->attr['type']]))  {
                            $result->addTriple(
                                    RDFtriple::page($pageID), 
                                    RDFtriple::URI("http://www.geonames.org/ontology#featureClass"),
                                    RDFtriple::URI("http://www.geonames.org/ontology#" . $this->typeToGeoNames[$g->attr['type']][0]));
                                    
                            if (isset($this->typeToGeoNames[$g->attr['type']][1]))
                                $result->addTriple(
                                        RDFtriple::page($pageID), 
                                        RDFtriple::URI("http://www.geonames.org/ontology#featureCode"),
                                        RDFtriple::URI("http://www.geonames.org/ontology#" 
                                                        . $this->typeToGeoNames[$g->attr['type']][0]
                                                        . '.' . $this->typeToGeoNames[$g->attr['type']][1]));
                        }
                        ...
                        if (strtolower($g->attr['type'] == 'city') && isset($g->attr['arg:type'])) {
                            $result->addTriple(
                                    RDFtriple::page($pageID), 
                                    RDFtriple::URI("http://www.geonames.org/ontology#population"),
                                    RDFtriple::Literal($g->attr['arg:type'], "http://www.w3.org/2001/XMLSchema#integer",NULL));
                        }
                        ...
                        if (strtolower($g->attr['type'] == 'landmark')) {
                            $result->addTriple(
                                    ...
                                    RDFtriple::URI("http://dbpedia.org/class/yago/Landmark108624891"));
                        }
                    } /* type */
     ...
     */
    private function extractGeoInfo($source, $titlesOnly = false)
    {
        foreach (($titlesOnly ? $this->knownTemplatesTitle : $this->knownTemplates) as $template) {
         if (preg_match($template, $source, $matches)) {
            return preg_split('/[|_]/', $matches[1]);
         }
        }
...
class geo_param {
    ...
    public function is_lat($lat)
    {
        return is_numeric($lat) && $lat >= -90 && $lat <= 90;
    }

    public function is_long($long)
    {
