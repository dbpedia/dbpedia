...
//		IF LINK IS AN IMAGE $c != 0
		...
		$x=$c?$prefix.encodeLocalName($img):$GLOBALS['W2RCFG']['wikipediaBase'].encodeLocalName($matches[1]);
		return true;
	}
...
function removeSemanticExtension(&$o) {
	
	$semanticPrefixes = array('mailto:[a-z\.A-Z@\-]+ ','mailto:','callto:\/\/[\+\-0-9]+ ','callto:\/\/');
	// Generate Literals from Semantic prefixes
	$o = preg_replace('/(\[)('.implode('|',$semanticPrefixes).') ?([^\]]+)(\])/','$3',$o);
	// Generate MediaWiki Internal Links
	$o = preg_replace('/(\[\[[a-zA-Z0-9\s]+::)([^\]\|]+)([^\]]*)(\]\])/','[[$2]]',$o);
	// Remove empty Semantic Links
	$o = preg_replace('/\[\[[a-zA-Z0-9\s]+::\]\]/','',$o);
	// Generate MediaWiki Literals
	$o = preg_replace('/(\[\[[a-zA-Z0-9\s]+:=)([^\]\|]+)([^\]]*)(\]\])/','$2',$o); 
	
	// Remove empty Properites
	$o = preg_replace('/(\[\[[a-zA-Z0-9\s]+:=)(\]\])/','',$o);
	// Remove Empty Image Links
	$o = preg_replace('/\[\[Image:\s*\]\]/','',$o);
	// Remove Empty Semantic Links
	$o = preg_replace('/(\[)('.implode('|',$semanticPrefixes).')(\])/','',$o); 
}
...
function catchExternalLink($o) {
	...
	if ( preg_match_all("/(http:\/\/[^ \]]+) ?([^\]]*\])/", $o, $matches) ) {
		return $matches;
	}
	return false;
}
