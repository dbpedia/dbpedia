...
class Indexer(val indexDir: File = LuceneConfig.defaultIndex) {
    ...
    private def getDocument(uriTerm: Term, fields: Map[String,Set[String]]): Document = {
        ...
        val sf = WikiUtil.wikiDecode(uriTerm.text.replace("http://dbpedia.org/resource/", "")).toLowerCase  //toLowerCase
        doc.add(new Field(LuceneConfig.Fields.SURFACE_FORM, sf, Field.Store.YES, Field.Index.NOT_ANALYZED, Field.TermVector.NO))

        for((field, valueSet) <- fields) {
            for(value <- valueSet) {
                if(field == LuceneConfig.Fields.SURFACE_FORM) {
                    //toLowerCase
                    doc.add(new Field(field, value.toLowerCase, Field.Store.YES, Field.Index.NOT_ANALYZED, Field.TermVector.NO))
                }
                else {
